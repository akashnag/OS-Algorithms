...
/* Implementation of the FIFO (First-In First-Out) Page Replacement Algorithm */
...
void add_new_frame(FrameBuffer*, int);
...
void add_new_frame(FrameBuffer *buffer, int frame)
{
	if(buffer->size < buffer->capacity)
	{
		...
		int i;
		for(i=0; i<buffer->size-1; i++) buffer->frames[i] = buffer->frames[i+1];
