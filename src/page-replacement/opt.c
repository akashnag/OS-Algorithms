...
/* Implementation of the OPT (Optimal) Page Replacement Algorithm */
...
void add_new_frame(FrameBuffer*, int*, int, int);
...
int main(void)
{
	...
	for(i=0; i<n; i++)
	{
		...
		{
			page_fault++;
			add_new_frame(buffer, ref_string, i, n);
		}
...
void add_new_frame(FrameBuffer *buffer, int *ref_string, int pos, int ref_string_size)
{
	if(buffer->size < buffer->capacity)
	{
		buffer->frames[buffer->size++] = ref_string[pos];
	} else {
		int i, j, sel_frame, sel_pos, max_pos, max_sel_frame_pos;
		...
		for(i=0; i<buffer->capacity; i++)
		{
			...
			sel_pos = ref_string_size;

			for(j=pos+1; j<ref_string_size; j++)
			{
		...
		buffer->frames[max_sel_frame_pos] = ref_string[pos];
	}
}