...
/* Implementation of the Worst-Fit Memory Allocation Algorithm */
...
int main(void)
{
	int i, n, process_size, hole_size, pos, max_hole_size;
	hole **list;
	...
	for(i=0; i<n; i++)
	{
		...
		{
			if(pos == -1 || hole_size > max_hole_size)
			{
				pos = i;
	...
	if(pos < 0)
		...
		printf("\nOriginal hole size: %d, new hole size: %d\n", max_hole_size, max_hole_size - process_size);
	}
