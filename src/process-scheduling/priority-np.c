...
/* Implementation of the non-preemptive Priority CPU Scheduling Algorithm */
...
void sort(process** list, int n)
{
	...
	for(i = 0; i < n - 1; i++)
	{
		for(j = 0; j < n - i - 1; j++)
		{
			if(list[j]->priority > list[j+1]->priority) swap(list, j, j+1);
		}
	}
}
...
void swap(process** list, int p1, int p2)
{
	...
	temp = list[p1]->priority;
	list[p1]->priority = list[p2]->priority;
	list[p2]->priority = temp;
}
...
void display(process** list, int n)
{
	...
	for(i = 0; i < n; i++)
	{
		printf("P%-2d\t%-8d\t%-5d\t%-7d\t%-10d\n", list[i]->pid, list[i]->priority, list[i]->burst, list[i]->waiting, list[i]->turnaround);
	}
}