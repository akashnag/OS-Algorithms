...
/* Implementation of the preemptive Priority CPU Scheduling Algorithm */
...
int get_highest_priority_process(process**, int, int);
...
int main(void)
{
	...
	while(completed < n)
	{
		k = get_highest_priority_process(list, n, time);
...
{
	int i;
	printf("\nPID\tArrival\tPriority\tBurst\tWaiting\tTurnaround\n---\t-------\t--------\t-----\t-------\t----------\n");
	for(i = 0; i < n; i++)
	{
		printf("P%-2d\t%-7d\t%-8d\t%-5d\t%-7d\t%-10d\n", list[i]->pid, list[i]->arrival, list[i]->priority, list[i]->burst, list[i]->waiting, list[i]->turnaround);
	}
}
...
int get_highest_priority_process(process** list, int n, int time)
{
	int i, pos;
	...
	for(i = 0; i<n; i++)
	{
		...
		{
			if(pos == -1 || list[i]->priority < list[pos]->priority)
				pos = i;
			else if(list[i]->priority == list[pos]->priority && list[i]->remaining_burst < list[pos]->remaining_burst)
				pos = i;
			else if(list[i]->priority == list[pos]->priority && list[i]->remaining_burst == list[pos]->remaining_burst && list[i]->arrival < list[pos]->arrival)
				pos = i;
		}
	}
